...
mod <- readRDS("/scratch/atm234/pinyon_jays/ebird/nospuncert/outputs/ebird_abund_model_checkN_habitatonly.RDS")
...
#a
a <- samp_df2 %>%
  dplyr::select('a[4]':'a[5]') %>%
  pivot_longer('a[4]':'a[5]',
               names_to = 'a',
               values_to = "value") %>%
  dplyr::select(value) %>%
  as_vector()
...
model <- jagsUI::jags(data = data_list,
                      ...
                      inits = inits_list2,
                      model.file = '/scratch/atm234/pinyon_jays/ebird/nospuncert/inputs/ebird_abund_JAGS_checkN_habitatonly.R',
                      parallel = TRUE,
                      n.chains = 3,
                      n.iter = 25000,
                      n.burnin = 5000,
...
saveRDS(model, 
        file ="/scratch/atm234/pinyon_jays/ebird/nospuncert/outputs/ebird_abund_model2_checkN_habitatonly.RDS")
...
mcmcplot(model$samples,
         dir = "/scratch/atm234/pinyon_jays/ebird/nospuncert/outputs/mcmcplots/checkN_run2_habitatonly")

...
mod <- readRDS("/scratch/atm234/pinyon_jays/ebird/nospuncert/outputs/ebird_abund_model_checkN_nocone.RDS")
...
#a
a <- samp_df2 %>%
  dplyr::select('a[1]':'a[4]') %>%
  pivot_longer('a[1]':'a[4]',
               names_to = 'a',
               values_to = "value") %>%
  dplyr::select(value) %>%
  as_vector()
...
model <- jagsUI::jags(data = data_list,
                      ...
                      inits = inits_list2,
                      model.file = '/scratch/atm234/pinyon_jays/ebird/nospuncert/inputs/ebird_abund_JAGS_no_cone.R',
                      parallel = TRUE,
                      n.chains = 3,
                      n.iter = 50000,
                      n.burnin = 15000,
...
saveRDS(model, 
        file ="/scratch/atm234/pinyon_jays/ebird/nospuncert/outputs/ebird_abund_model2_checkN_nocone.RDS")
...
mcmcplot(model$samples,
         dir = "/scratch/atm234/pinyon_jays/ebird/nospuncert/outputs/mcmcplots/checkN_nocone_run2")
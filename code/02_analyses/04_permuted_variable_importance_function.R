...
#THESE WILL NEED TO BE RESHUFFLED IN EACH RUN - need a model argument for this
...
predict_fun <- function(covariate){
  
  #shuffle the covariate of interest
  shuffled_cone <- array(sample(Cone), dim=dim(Cone))
  shuffled_temp <- array(sample(Temp), dim=dim(Temp))
  shuffled_ppt <- array(sample(PPT), dim=dim(PPT))
  ...
  Cone <- if(covariate == "Cone") shuffled_cone else cov_list[["Cone"]]
  Temp <- if(covariate == "Temp") shuffled_temp else cov_list[["Temp"]]
  PPT <- if(covariate == "PPT") shuffled_ppt else cov_list[["PPT"]]
...
cov_vector <- c(rep("Cone", 20),
                rep("Temp", 20),
                rep("PPT", 20),
                rep("Monsoon", 20),
                rep("PinyonBA", 20))

importance_df_list <- lapply(cov_vector, predict_fun)

importance_df <- bind_rows(importance_df_list)

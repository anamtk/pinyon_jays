...
ebird_cells <- as.data.frame(ebird) %>%
  distinct(cellID, year) %>%
  group_by(year) %>%
  mutate(ebirdnumID = 1:n()) %>%
  ungroup() %>%
  rename(ebirdcellID = cellID)
...
n.ebird.check <- as.data.frame(ebird3) %>%
  dplyr::select(cellID, chckls_, year) %>%
  left_join(ebird_cells, by = c("cellID" ='ebirdcellID', "year")) %>%
  group_by(ebirdnumID, year) %>%
  tally() %>%
  pivot_wider(names_from = ebirdnumID, 
              values_from = n) %>%
  column_to_rownames(var= 'year') %>%
  as.matrix()
...
grid <- ebird_index_df$ebirdnumID
...
for(i in 1:dim(ebird_index_df)[1]){ #dim[1] = n.rows
  ...
  ebird.count[yr2[i], grid[i], check[i]] <- as.numeric(ebird_index_df[i,3])
}
...
ebird_index2 <- ebird_index_df %>%
  distinct(ebirdnumID, ebirdcellID)
...
ebird_grid_df <- ebird_gridIDs %>%
  ...
  left_join(ebird_cells, by = c("ebirdcellID", 'year')) %>%
  group_by(yearID, ebirdnumID) %>%
  mutate(piID = 1:n()) %>%
  ungroup() %>%
  dplyr::select(ebirdcellID, year, cell, prop, numID,
                yearID, ebirdnumID, piID)

egridyr <- ebird_grid_df$yearID
egridcell <- ebird_grid_df$ebirdnumID
egridid <- ebird_grid_df$piID

ebird.pi <- array(NA, dim = c(n.years, max(egridcell), max(egridid)))
...
for(i in 1:dim(ebird_grid_df)[1]){ #dim[1] = n.rows
  ...
  ebird.pi[egridyr[i], egridcell[i], egridid[i]] <- as.numeric(ebird_grid_df[i,4])
}
...
ebird.grid.array <- array(NA, dim = c(n.years, max(egridcell), max(egridid)))
...
for(i in 1:dim(ebird_grid_df)[1]){ #dim[1] = n.rows
  ...
  # the dataframe that corresponds to the pi data
  # for that yearxtransectxpi combo
  ebird.grid.array[egridyr[i], egridcell[i], egridid[i]] <- as.numeric(ebird_grid_df[i,5])
}

#matrix t,i
n.ebird.cells <- ebird_grid_df %>%
  distinct(yearID, ebirdnumID, piID) %>%
  group_by(yearID, ebirdnumID) %>%
  tally() %>%
  arrange(ebirdnumID) %>%
  pivot_wider(names_from = ebirdnumID,
